...
public class PaginatedListSmartListHelper extends SmartListHelper
{
    ...
    public PaginatedListSmartListHelper(PaginatedList paginatedList, TableProperties tableProperties, PageContext pageContext)
    {
        super();
    ...
    public String getPageNavigationBar(Href baseHref, String pageParameter)
    {
        ...
        {
            bannerFormat = this.properties.getPagingBannerOnePage(pageContext);
        }
        else if (pagination.isFirst())
        {
            bannerFormat = this.properties.getPagingBannerFirst(pageContext);
        }
        else if (pagination.isLast())
        {
            bannerFormat = this.properties.getPagingBannerLast(pageContext);
        }
        else
        {
            bannerFormat = this.properties.getPagingBannerFull(pageContext);
        }

        return pagination.getFormattedBanner(this.properties.getPagingPageLink(pageContext), this.properties
            .getPagingPageSelected(pageContext), this.properties.getPagingPageSeparator(pageContext), bannerFormat);
    }
    ...
    public String getSearchResultsSummary()
    {
        ...
        {
            objs = new Object[]{this.properties.getPagingItemsName(pageContext)};
            message = this.properties.getPagingFoundNoItems(pageContext);
        }
        else if (this.paginatedList.getFullListSize() == 1)
        {
            objs = new Object[]{this.properties.getPagingItemName(pageContext)};
            message = this.properties.getPagingFoundOneItem(pageContext);
        }
        else if (pageCount == 1)
        {
            objs = new Object[]{
                new Integer(this.paginatedList.getFullListSize()),
                this.properties.getPagingItemsName(pageContext),
                this.properties.getPagingItemsName(pageContext)};
            message = this.properties.getPagingFoundAllItems(pageContext);
        }
        else
        {
            objs = new Object[]{
            ...
            message = this.properties.getPagingFoundSomeItems(pageContext);
        }
